								...
								<div class="card">
									<div class="card-header">
										<div class="card-head-row">
											<div class="card-title">Pending Appointments List <span class="badge badge-secondary">{{appointmentStats.pending}}</span></div>
									...
									<div class="card-body">
										<table class="table table-head-bg-secondary" >
											...
											<tbody>
												<tr v-for="(appointment,index) in filteredPendingAppointments" >
										...
										<div v-show="filteredPendingAppointments.length == 0" class="alert alert-secondary" role="alert">No pending appointments found.</div>
...
export default {
  ...
  computed: {
	isPendingAppointmentsQueue (){
		return this.$store.getters['appointment/getPending'].length !==0
	},
	pendingAppointments (){
		return this.$store.getters['appointment/getPending']
	},
	...
	methods: {
			...
			if(!this.isClicked){
				...
				appointmentApi.acceptAppointment(id).then(response=> {
					if(response.success) {
						this.$store.dispatch('appointment/getPending')
		...
		searchAppointments() {
			this.$store.dispatch('appointment/searchAppointments', this.dateRange)
		},
		updateStatus(id) {
			...
			if (bottomOfWindow) {
				this.loadingData = true
					if(this.page < this.lastPage) {
							this.page++
							this.$store.dispatch ( 'appointment/getMorePending', this.page ).then ( response => {
	...
	mounted () {
	if(!this.isPendingAppointmentsQueue){
		this.$store.dispatch('appointment/getPending', this.page).then(response => {
