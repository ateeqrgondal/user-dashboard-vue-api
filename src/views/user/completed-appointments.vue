				...
				<div class="content">
					<div class="page-inner">
						<div class="page-header">
							<h4 class="page-title">Completed Appointments</h4>
								...
								<div class="card">
									<div class="card-header">
										<div class="card-head-row">
											<div class="card-title">Completed Appointments List <span class="badge badge-secondary">{{appointmentStats.completed}}</span></div>
									...
									<div class="card-body">
										<table class="table table-head-bg-secondary" >
											...
											<tbody>
												<tr v-for="(appointment,index) in filteredAppointments" >
										...
										<div v-show="filteredAppointments.length == 0" class="alert alert-secondary" role="alert">No pending appointments found.</div>
...
export default {
  ...
  computed: {
	isCompletedAppointments (){
		return this.$store.getters['appointment/getCompleted'].length !==0
	},
	completedAppointments (){
		return this.$store.getters['appointment/getCompleted']
	},
	filteredAppointments() {
		return this.completedAppointments.filter(appointment => {
	...
	methods: {
			...
			if(!this.isClicked){
				...
				appointmentApi.acceptAppointment(id).then(response=> {
					if(response.success) {
						this.$store.dispatch('appointment/getCompleted')
		...
		searchAppointments() {
			this.$store.dispatch('appointment/searchCompletedAppointments', this.dateRange)
		},
		updateStatus(id) {
			...
			if (bottomOfWindow) {
				this.loadingData = true
					if(this.page < this.lastPage) {
							this.page++
							this.$store.dispatch ( 'appointment/getMoreCompleted', this.page ).then ( response => {
	...
	mounted () {
	if(!this.isCompletedAppointments){
		this.$store.dispatch('appointment/getCompleted', this.page).then(response => {
