								...
								<div class="card">
									<div class="card-header">
										<div class="card-head-row">
											<div class="card-title">
														<div class="user ">
															<div class="avatar">
																<span class="avatar-title rounded-circle border border-white text-uppercase"><span class="fa fa-envelope-open"></span></span>
															</div>
														</div>
									...
									<div class="card-footer" style="background: #f1f1f1" :class="{'is-loading' : loading == true}">
										...
										<div class="input-file input-file-image text-right" style="margin-right: 10px">
											<input type="file" class="form-control form-control-file" id="uploadImg2" name="profile picture" v-validate="'ext:jpeg,jpg,png'" data-vv-as="field"  @change="uploadAttachment($event)" >
...
export default {
    ...
    watch: {
		chatHead () {
					this.$store.dispatch('user/getUser', this.chatHead.receiverId)
		},
    },
	methods: {
		moment (moment) {
			...
			this.$validator.validateAll().then(result => {
				if(result){
					...
					chatApi.sendMessage(formData).then ( response => {
						if ( response.success ) {
							this.$store.dispatch('chat/getChatHead', this.$route.params.id)
							this.loading   = false
							this.resetForm()
						}
					} )
