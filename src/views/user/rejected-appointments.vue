				...
				<div class="content">
					<div class="page-inner">
						<div class="page-header">
							<h4 class="page-title">Rejected Appointments</h4>
								...
								<div class="card">
									<div class="card-header">
										<div class="card-head-row">
											<div class="card-title">Rejected Appointments List <span class="badge badge-secondary">{{appointmentStats.rejected}}</span></div>
									...
									<div class="card-body">
										<table class="table table-head-bg-secondary" >
											...
											<tbody>
												<tr v-for="(appointment,index) in filteredRejectedAppointments" >
										...
										<div v-show="filteredRejectedAppointments.length == 0" class="alert alert-secondary" role="alert">No rejected appointments found.</div>
...
export default {
  ...
  computed: {
	isRejectedAppointments (){
		return this.$store.getters['appointment/getRejected'].length !==0
	},
	rejectedAppointments (){
		return this.$store.getters['appointment/getRejected']
	},
	...
	methods: {
			...
			if(!this.isClicked){
				...
				appointmentApi.acceptAppointment(id).then(response=> {
					if(response.success) {
						this.$store.dispatch('appointment/getRejected')
		...
		searchAppointments() {
			this.$store.dispatch('appointment/searchRejectedAppointments', this.dateRange)
		},
		updateStatus(id) {
			...
			if (bottomOfWindow) {
				this.loadingData = true
					if(this.page < this.lastPage) {
							this.page++
							this.$store.dispatch ( 'appointment/getMoreRejected', this.page ).then ( response => {
	...
	mounted () {
	if(!this.isRejectedAppointments){
		this.$store.dispatch('appointment/getRejected', this.page).then(response => {
