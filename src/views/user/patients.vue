								...
								<div class="card">
									<div class="card-header">
										<div class="card-head-row">
											<div class="card-title">Patients who took appointments</div>
									...
									<div class="card-body">
										...
										<div v-show="filteredPatients.length == 0" class="alert alert-secondary" role="alert">No patients info found. Please add in the list.</div>
...
export default {
  ...
  computed: {
	patients (){
		return this.$store.getters['common/getPatients']
	},
	filteredPatients () {
		return this.patients.filter(patient => {
			return patient.first_name.toLowerCase().includes(this.filter.toLowerCase().trim()) |
					patient.last_name.toLowerCase().includes(this.filter.toLowerCase().trim()) |
					(patient.first_name + ' ' + patient.last_name).toLowerCase().includes(this.filter.toLowerCase().trim())
		})
	}
  },
  methods: {
	getPatients() {
		this.$store.dispatch('common/getPatients')
	},
  },
	mounted () {
	this.getPatients()
