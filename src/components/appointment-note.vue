	...
	<div class="row">
		<div class="col-md-12">
			<div class="form-group " :class="{'has-error has-feedback' : errors.has('note')}">
				...
				<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="Reason of cancellation" v-model="appointmentNote.rejectNote"  v-validate="'required'" name="note"></textarea>
	...
	methods: {
		addNote() {
			...
			this.$validator.validateAll().then((result) => {
				if (result) {
					this.loading = true
					appointmentApi.cancelAppointment ( this.appointment.id, this.appointmentNote ).then ( response => {
						if ( response.success ) {
